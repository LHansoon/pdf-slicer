...
done < ../worker/worker-docker.env

last_line=$(grep "." ../worker/worker-docker.env | tail -1)
...
aws elasticbeanstalk update-environment --environment-name Pdfworkerup-env --option-settings file://credentials.json --region us-east-1 > /dev/null
rm credentials.json
